...
class VirtualInstrument(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Virtual Nuclear Radiation Counting System - Dead Time Simulator")
        self.geometry("1250x850")
        self.configure(bg="#2b2b2b") 

        # --- PHYSICAL VARIABLES ---
        self.true_rate = 100.0          # True pulse rate (cps)
        self.tau = 0.005                # Default dead time (5 ms)
    ...
    def setup_ui(self):
        ...
        # ================= MODULE 1: PULSE GENERATOR =================
        gen_frame = tk.LabelFrame(control_frame, text=" RANDOM PULSE GENERATOR ", bg="#3c3f41", fg="#a9b7c6", font=label_font)
        ...
        tk.Label(input_frame, text="Rate (cps):", bg="#3c3f41", fg="white").pack(side="left")
        ...
        # ================= MODULE 2: PULSE COUNTER =================
        count_frame = tk.LabelFrame(control_frame, text=" RADIATION COUNTER ", bg="#3c3f41", fg="#a9b7c6", font=label_font)
        ...
        # Dead time (tau) input frame
        tau_input_frame = tk.Frame(count_frame, bg="#3c3f41")
        tau_input_frame.pack(fill="x", padx=10, pady=(5,0))
        tk.Label(tau_input_frame, text="Processing time \u03C4 (ms):", bg="#3c3f41", fg="white").pack(side="left")
        ...
        ttk.Radiobutton(count_frame, text="Non-paralyzable (fixed)", variable=self.mode_var, value="non", command=self.force_redraw).pack(anchor="w", padx=10, pady=2)
        ttk.Radiobutton(count_frame, text="Paralyzable (extendable)", variable=self.mode_var, value="par", command=self.force_redraw).pack(anchor="w", padx=10, pady=2)
        ...
        # ================= MODULE 3: PLOT SETTINGS =================
        graph_ctrl_frame = tk.LabelFrame(control_frame, text=" PLOT DISPLAY SETTINGS ", bg="#3c3f41", fg="#a9b7c6", font=label_font)
        ...
        tk.Label(win_frame, text="Pulses shown \nin the plot window:", bg="#3c3f41", fg="white", justify="left").pack(side="left")
    ...
    def on_rate_entry(self, event=None):
        try:
            ...
            val = max(1, min(50000, val)) # Limited to 1-50000 cps
            self.true_rate = val
    ...
    def on_tau_entry(self, event=None):
        try:
            ...
            val = max(0.001, min(300.0, val)) # Limited to 0.1-300 ms
            self.tau = val / 1000.0
    ...
    def draw_graphs(self):
        ...
        self.ax1.set_title(f"PULSES ARRIVING AT THE DETECTOR ({self.count_real} pulses generated)", color='white', fontsize=10)
        self.ax1.set_ylim(0, 1.2)
        self.ax1.set_yticks([])
        self.ax1.grid(color='#444444', linestyle='--', alpha=0.5)

        mode = self.mode_var.get()
        self.ax2.vlines(ev_real_plot, 0, 0.4, color='gray', linestyle=':', label='Incoming pulse')

        if mode == "non":
            ev_plot = [e for e in self.events_non if min_t <= e <= max_t]
            blocks = self.blocks_non
            title = "DEAD-TIME WINDOWS (NON-PARALYZABLE)"
            color = 'orange'
        else:
            ev_plot = [e for e in self.events_par if min_t <= e <= max_t]
            blocks = self.blocks_par
            title = "DEAD-TIME WINDOWS (PARALYZABLE)"
            color = 'red'

        self.ax2.vlines(ev_plot, 0, 1, color='#00ff00', lw=2, label='Recorded pulse')
        ...
        self.ax2.set_yticks([])
        self.ax2.set_xlabel("Elapsed time (s)", color='white')
